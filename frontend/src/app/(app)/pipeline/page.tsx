...
export default function PipelinePage() {
  ...
  // We still want to show the button even if there are no candidates yet
  if (isLoading) {
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold tracking-tight text-foreground">Pipeline Explorer</h1>
      </div>

      {(!candidates || candidates.length === 0) ? (
        <EmptyState message="No candidates have been processed yet. Click Start Pipeline to begin." />
      ) : (
        <>
          {isStale(candidates[0].updated_at) && <StaleDataBanner />}
          <div className="flex flex-1 flex-col gap-4 lg:flex-row lg:overflow-hidden">
            <div className="shrink-0 lg:w-72 lg:overflow-y-auto">
              ...
              <CandidateList
                ...
                selectedSymbol={selectedSymbol ?? candidates[0].symbol}
            ...
            <div className="min-w-0 flex-1 lg:overflow-y-auto lg:pr-2">
              ...
              <PipelineStepper candidate={candidates.find((c) => c.symbol === selectedSymbol) ?? candidates[0]} />
            </div>
          </div>
        </>
      )}
